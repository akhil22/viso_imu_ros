#ifndef IMU_STEREO_PROCESSOR_H_
#define IMU_STEREO_PROCESSOR_H_
...
private:
  ...
  int left_received_, right_received_, left_info_received_, right_info_received_, imu_received_ ,all_received_;
...
protected:
  ...
  StereoProcessor(const std::string& transport) :
    left_received_(0), right_received_(0), left_info_received_(0), right_info_received_(0),imu_received_(0), all_received_(0)
  {
    ...
    ROS_INFO("Subscribing to:\n\t* %s\n\t* %s\n\t* %s\n\t* %s\n\t* %s",
        ...
        left_info_topic.c_str(), right_info_topic.c_str(),imu_topic.c_str());
    ...
    imu_sub_.subscribe(nh, imu_topic, 3);
    ...
    imu_sub_.registerCallback(boost::bind(StereoProcessor::increment,&imu_received_));
    ...
    if (approx)
    {
      approximate_sync_.reset(new ApproximateSync(ApproximatePolicy(queue_size_),
                                                  left_sub_, right_sub_, left_info_sub_, right_info_sub_, imu_sub_) );
